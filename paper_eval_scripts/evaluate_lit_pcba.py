...
proteins = os.listdir("LIT-PCBA-fasta")
...
for protein in sorted(proteins):
    ...
    sequences = [str(x.seq) for x in list(SeqIO.parse("LIT-PCBA-fasta/" + protein, "fasta"))]
    ...
    actives = [x.split() for x in open("LIT-PCBA-smiles/" + dud_id + "_actives.smi").read().split("\n") if len(x.split()) > 0]
    decoys = [x.split() for x in open("LIT-PCBA-smiles/" + dud_id + "_inactives.smi").read().split("\n") if len(x.split()) > 1]
    ...
    open("LIT-PCBA_results/" + dud_id + ".json", "w+").write(json.dumps(writable, indent=4))