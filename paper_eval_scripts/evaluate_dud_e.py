...
for protein in sorted(proteins):
    ...
    actives = [x.split() for x in open("DUD-E_smiles/" + dud_id + "_actives.smi").read().split("\n") if len(x.split()) > 0]
    decoys = [x.split() for x in open("DUD-E_smiles/" + dud_id + "_decoys.smi").read().split("\n") if len(x.split()) > 1]
    ...
    open("DUD-E_results/" + dud_id + ".json", "w+").write(json.dumps(writable, indent=4))
