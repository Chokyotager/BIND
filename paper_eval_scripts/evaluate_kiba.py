...
kiba_data = list(csv.reader(open("davis_kiba/kiba.csv")))[1:]

proteins = list(set([x[4] for x in kiba_data]))
protein_states = dict()
...
for kiba_datapoint in tqdm(kiba_data):

    smiles = kiba_datapoint[2]
    graph = get_graph(smiles)

    protein_sequence = kiba_datapoint[4]
