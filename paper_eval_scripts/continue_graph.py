...
writer = SummaryWriter(log_dir="/home/hilbert/BIND/runs/BIND_continued")
...
continue_from = 27000
last_save_file = "saves/model_27000.pth"
...
model = torch.load(last_save_file)
...
optimiser = torch.optim.AdamW(model.parameters(), lr=learning_rate, weight_decay=1e-9)
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimiser, T_max=iterations)

for i in range(continue_from):
    scheduler.step()
...
with torch.backends.cuda.sdp_kernel(enable_flash=True, enable_math=False, enable_mem_efficient=False):
    ...
    for i in range(continue_from, iterations):
        ...
        try:
            ...
            for j in range(grad_accumulation_steps):
                ...
                (current_loss / grad_accumulation_steps).backward()
