...
proteins = os.listdir("CASF-2016_fasta")
...
for protein in sorted(proteins):
    ...
    sequences = [str(x.seq) for x in list(SeqIO.parse("CASF-2016_fasta/" + protein, "fasta"))]
    ...
    actives = [x.split() for x in open("CASF-2016_smiles/" + dud_id + "_actives.smi").read().split("\n") if len(x.split()) > 1]
    decoys = [x.split() for x in open("CASF-2016_smiles/" + dud_id + "_inactives.smi").read().split("\n") if len(x.split()) > 1]
    ...
    open("CASF_results/" + dud_id + ".json", "w+").write(json.dumps(writable, indent=4))
