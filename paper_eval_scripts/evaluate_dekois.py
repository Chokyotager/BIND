...
model = torch.load("model_40000.pth", map_location=device)
model.to(device)

model.eval()

proteins = os.listdir("DEKOIS_fasta")
...
for protein in sorted(proteins):
    ...
    sequence = str(list(SeqIO.parse("DEKOIS_fasta/" + protein, "fasta"))[0].seq)
    ...
    actives = [x.split() for x in open("DEKOIS_smiles/" + dud_id + ".sdf.actives.smi").read().split("\n") if len(x.split()) > 1]
    decoys = [x.split() for x in open("DEKOIS_smiles/" + dud_id + "_Celling-v1.12_decoyset.sdf.decoys.smi").read().split("\n") if len(x.split()) > 1]
    ...
    open("DEKOIS_results/" + dud_id + ".json", "w+").write(json.dumps(writable, indent=4))
